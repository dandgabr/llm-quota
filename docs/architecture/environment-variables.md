...
| `ENABLE_DEV_SESSION`  | dev-only `/auth/*` endpoints                  | unset (off)    | no              | `1` enables `POST /auth/issue-session` + OIDC/MFA challenge endpoints — **and only** with `NODE_ENV` ≠ production. Fail-closed. |
...
| `COLLECT_INTERVAL_MS` | collector scheduler (in-process)              | `60000`        | no              | First pass ~5 s after boot. `0` disables the scheduler.                                                        |
